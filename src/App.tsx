import { useState } from "react";
import { GoDotFill, GoHeartFill } from "react-icons/go";
import { IoMenu } from "react-icons/io5";
import "./styles/style.scss";

const Socials = () => {
  return (
    <div className="socials">
      <svg
        width="23"
        height="18"
        viewBox="0 0 23 18"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
      >
        <path
          d="M22.9617 2.11613C22.1435 2.475 21.2718 2.71288 20.3629 2.82838C21.2979 2.27013 22.0115 1.39287 22.347 0.3355C21.4753 0.85525 20.5128 1.22238 19.487 1.42725C18.6593 0.545875 17.4795 0 16.1925 0C13.6955 0 11.6853 2.02675 11.6853 4.51137C11.6853 4.86888 11.7155 5.21263 11.7898 5.53988C8.04017 5.357 4.72229 3.55988 2.49342 0.82225C2.10429 1.49738 1.87604 2.27012 1.87604 3.102C1.87604 4.664 2.68042 6.04862 3.87942 6.85025C3.15479 6.8365 2.44392 6.62613 1.84167 6.29475C1.84167 6.3085 1.84167 6.32638 1.84167 6.34425C1.84167 8.536 3.40505 10.3565 5.45517 10.7759C5.08805 10.8763 4.68792 10.9244 4.27267 10.9244C3.98392 10.9244 3.69242 10.9079 3.41879 10.8474C4.00317 12.6335 5.66142 13.9466 7.63317 13.9893C6.09867 15.1896 4.1503 15.9129 2.04104 15.9129C1.67117 15.9129 1.31642 15.8964 0.96167 15.851C2.95954 17.1394 5.3273 17.875 7.88067 17.875C16.1802 17.875 20.7177 11 20.7177 5.04075C20.7177 4.84137 20.7108 4.64887 20.7012 4.45775C21.5963 3.8225 22.3484 3.02913 22.9617 2.11613Z"
          fill="black"
        />
      </svg>
      <svg
        width="10"
        height="20"
        viewBox="0 0 10 20"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
      >
        <path
          d="M8.11163 3.29509H9.92331V0.139742C9.61075 0.0967442 8.53581 0 7.28393 0C4.67183 0 2.88248 1.643 2.88248 4.66274V7.44186H0V10.9693H2.88248V19.845H6.41654V10.9701H9.18243L9.6215 7.44269H6.41571V5.01251C6.41654 3.99297 6.69106 3.29509 8.11163 3.29509Z"
          fill="black"
        />
      </svg>
      <svg
        width="23"
        height="22"
        viewBox="0 0 23 22"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
      >
        <path
          fill-rule="evenodd"
          clip-rule="evenodd"
          d="M7.83667 0H16.0867C19.883 0 22.9617 3.07862 22.9617 6.875V15.125C22.9617 18.9214 19.883 22 16.0867 22H7.83667C4.04029 22 0.96167 18.9214 0.96167 15.125V6.875C0.96167 3.07862 4.04029 0 7.83667 0ZM16.0867 19.9375C18.7404 19.9375 20.8992 17.7787 20.8992 15.125V6.875C20.8992 4.22125 18.7404 2.0625 16.0867 2.0625H7.83667C5.18292 2.0625 3.02417 4.22125 3.02417 6.875V15.125C3.02417 17.7787 5.18292 19.9375 7.83667 19.9375H16.0867Z"
          fill="black"
        />
        <path
          fill-rule="evenodd"
          clip-rule="evenodd"
          d="M6.46167 11C6.46167 7.96263 8.92429 5.5 11.9617 5.5C14.999 5.5 17.4617 7.96263 17.4617 11C17.4617 14.0374 14.999 16.5 11.9617 16.5C8.92429 16.5 6.46167 14.0374 6.46167 11ZM8.52417 11C8.52417 12.8948 10.0669 14.4375 11.9617 14.4375C13.8564 14.4375 15.3992 12.8948 15.3992 11C15.3992 9.10388 13.8564 7.5625 11.9617 7.5625C10.0669 7.5625 8.52417 9.10388 8.52417 11Z"
          fill="black"
        />
        <circle cx="17.8742" cy="5.08749" r="0.732875" fill="black" />
      </svg>
    </div>
  );
};

const HeaderMenu = () => {
  return (
    <ul>
      <li>
        <a href="">Home</a>
      </li>
      <li>
        <a href="">Recipes</a>
      </li>
      <li>
        <a href="">Blog</a>
      </li>
      <li>
        <a href="">Contact</a>
      </li>
      <li>
        <a href="">About us</a>
      </li>
    </ul>
  );
};

const categories = [
  { title: "Breakfast", src: "/categories/image 25.png" },
  { title: "Vegan", src: "/categories/image 20.png" },
  { title: "Meat", src: "/categories/image 21.png" },
  { title: "Dessert", src: "/categories/image 22.png" },
  { title: "Lunch", src: "/categories/image 23.png" },
  { title: "Chocolate", src: "/categories/image 24.png" },
];

const simpleRecipes = [
  {
    title: "Big and Juicy Wagyu Beef Cheeseburger",
    src: "/simple_recipes/image 26 (7).png",
    category: "Snack",
  },
  {
    title: "Fresh Lime Roasted Salmon with Ginger Sauce",
    src: "/simple_recipes/image 26.png",
    category: "Fish",
  },
  {
    title: "Strawberry Oatmeal Pancake with Honey Syrup",
    src: "/simple_recipes/image 26 (1).png",
    category: "Breakfast",
  },
  {
    title: "Fresh and Healthy Mixed Mayonnaise Salad",
    src: "/simple_recipes/image 26 (2).png",
    category: "Healthy",
  },
  {
    title: "Chicken Meatballs with Cream Cheese",
    src: "/simple_recipes/image 26 (3).png",
    category: "Meat",
  },
  {
    title: "Fruity Pancake with Orange & Blueberry",
    src: "/simple_recipes/image 26 (4).png",
    category: "Sweet",
  },
  {
    title: "The Best Easy One Pot Chicken and Rice",
    src: "/simple_recipes/image 26 (5).png",
    category: "Snack",
  },
  {
    title: "The Creamiest Creamy Chicken and Bacon Pasta",
    src: "/simple_recipes/image 26 (6).png",
    category: "Noodles",
  },
  {
    title: "Barbeque Spicy Sandwiches with Chips ",
    src: "/simple_recipes/image 26 (8).png",
    category: "Snack",
  },
];

const deliciousRecipes = [
  {
    title: "Mixed Tropical Fruit Salad with Superfood Boosts",
    src: "/delicious_recipes/image 26 (1).png",
    category: "Healthy",
  },
  {
    title: "Big and Juicy Wagyu Beef Cheeseburger",
    src: "/delicious_recipes/image 26 (3).png",
    category: "Western",
  },
  {
    title: "Healthy Japanese Fried Rice with Asparagus",
    src: "/delicious_recipes/image 26.png",
    category: "Healthy",
  },
  {
    title: "Cauliflower Walnut Vegetarian Taco Meat",
    src: "/delicious_recipes/image 26 (2).png",
    category: "Eastern",
  },
  {
    title: "Rainbow Chicken Salad with Almond Honey Mustard Dressing",
    src: "/delicious_recipes/image 26 (4).png",
    category: "Healthy",
  },
  {
    title: "Barbeque Spicy Sandwiches with Chips",
    src: "/delicious_recipes/image 26 (8).png",
    category: "Snack",
  },
  {
    title: "Firecracker Vegan Lettuce Wraps - Spicy!",
    src: "/delicious_recipes/image 26 (5).png",
    category: "Seafood",
  },
  {
    title: "Chicken Ramen Soup with Mushroom",
    src: "/delicious_recipes/image 26 (6).png",
    category: "Japanese",
  },
];
const posts = [
  {
    title: "Mixed Tropical Fruit Salad with Superfood Boosts",
    src: "/instagram/Rectangle.png",
    category: "Healthy",
  },
  {
    title: "Big and Juicy Wagyu Beef Cheeseburger",
    src: "/instagram/Rectangle (1).png",
    category: "Western",
  },
  {
    title: "Healthy Japanese Fried Rice with Asparagus",
    src: "/instagram/Rectangle (2).png",
    category: "Healthy",
  },
  {
    title: "Cauliflower Walnut Vegetarian Taco Meat",
    src: "/instagram/Rectangle (3).png",
    category: "Eastern",
  },
];

export default function App() {
  const [openMenuModal, setOpenMenuModal] = useState(false);

  return (
    <div>
      <header>
        <h1>
          Foodieland<span>.</span>
        </h1>
        <HeaderMenu />
        <Socials />
        <div className="hamburger_menu" onClick={() => setOpenMenuModal(true)}>
          <IoMenu size={20} />
        </div>
      </header>

      {/* Menu List */}
      <div className={`toggle_menu ${openMenuModal ? "menu-active" : ""}`}>
        <HeaderMenu />
        <div className="socials_container">
          <Socials />
        </div>
        <span onClick={() => setOpenMenuModal(false)}>x</span>
      </div>

      {/* Hero */}
      <div className="hero_container">
        <div className="hero">
          <div className="handpick_recipes">
            <img src="/hero/thumbup.png" alt="" />
            <svg
              width="150"
              height="150"
              viewBox="0 0 150 150"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
              xmlnsXlink="http://www.w3.org/1999/xlink"
            >
              <path
                d="M-6.55671e-06 75C-1.01779e-05 33.5787 33.5786 1.01779e-05 75 6.55671e-06C116.421 2.93554e-06 150 33.5786 150 75C150 116.421 116.421 150 75 150C33.5786 150 -2.93554e-06 116.421 -6.55671e-06 75Z"
                fill="black"
              />
              <path
                d="M35 75C35 52.9086 52.9086 35 75 35C97.0914 35 115 52.9086 115 75C115 97.0914 97.0914 115 75 115C52.9086 115 35 97.0914 35 75Z"
                fill="white"
              />
              <path
                d="M123.541 39.7329C128.826 47.0068 132.408 55.3755 134.024 64.2204C135.639 73.0652 135.246 82.1599 132.873 90.8324C130.501 99.5048 126.21 107.533 120.317 114.324C114.424 121.115 107.081 126.494 98.8289 130.065C90.5772 133.636 81.6286 135.307 72.6444 134.954C63.6602 134.601 54.8702 132.233 46.9242 128.026C38.9782 123.819 32.0793 117.879 26.7375 110.647C21.3957 103.415 17.7475 95.0747 16.0628 86.2429"
                stroke="white"
                stroke-width="12"
                stroke-dasharray="1 10"
              />
              <path
                d="M19.3772 74.6013L19.5123 72.761L15.9169 72.497L16.1912 68.7614L19.7866 69.0254L19.9214 67.1894L11.2176 66.5503L11.0828 68.3863L14.674 68.65L14.3997 72.3856L10.8085 72.122L10.6734 73.9622L19.3772 74.6013Z"
                fill="white"
              />
              <path
                d="M20.3807 64.8451L18.583 63.7736L19.2789 60.7023L21.3629 60.5103L21.7998 58.5819L12.6235 59.5875L12.098 61.9066L19.9438 66.7735L20.3807 64.8451ZM17.2819 62.9981L14.2983 61.221L14.3133 61.1545L17.7715 60.837L17.2819 62.9981Z"
                fill="white"
              />
              <path
                d="M16.5008 48.2084L15.8298 49.9181L20.9391 51.9232L20.9111 51.9946L14.4257 53.4962L13.8341 55.0036L21.9582 58.1918L22.6323 56.4741L17.519 54.4675L17.5424 54.408L24.0427 52.8801L24.6249 51.3965L16.5008 48.2084Z"
                fill="white"
              />
              <path
                d="M26.5957 47.8202C27.9228 45.5159 27.3013 43.2991 24.938 41.938C22.582 40.5811 20.3601 41.1603 19.0458 43.4424L17.489 46.1455L25.0517 50.5011L26.5957 47.8202ZM24.6025 48.1131L19.7799 45.3356L20.3668 44.3164C21.1835 42.8984 22.339 42.5656 24.0192 43.5333C25.7068 44.5052 26.0062 45.6759 25.1874 47.0976L24.6025 48.1131Z"
                fill="white"
              />
              <path
                d="M29.3533 43.2632L30.4924 41.8116L28.2665 40.0648L29.2241 38.8445C30.4632 37.2656 30.242 35.6078 28.8709 34.5318C27.5065 33.4611 25.8367 33.6078 24.6134 35.1667L22.4877 37.8754L29.3533 43.2632ZM27.1032 39.1519L24.8136 37.3551L25.5817 36.3762C26.2394 35.5381 27.0137 35.4903 27.6976 36.027C28.3781 36.561 28.5291 37.3349 27.8767 38.1663L27.1032 39.1519Z"
                fill="white"
              />
              <path
                d="M29.106 30.0623L27.83 31.3952L34.1342 37.4303L35.4102 36.0974L29.106 30.0623Z"
                fill="white"
              />
              <path
                d="M38.8089 25.5483C37.3373 24.1496 35.423 24.2284 33.8663 25.5155C32.0896 26.9845 31.7705 29.3606 33.5898 31.561C35.4036 33.7548 37.7797 33.9135 39.5795 32.4255C41.3037 30.9999 41.3971 28.9929 40.5277 27.6272L39.0838 28.8099C39.5439 29.6405 39.3211 30.5214 38.5625 31.1486C37.5411 31.9931 36.2209 31.8074 35.0315 30.3689C33.8748 28.9698 33.898 27.607 34.9391 26.7462C35.7175 26.1027 36.6511 26.0883 37.3704 26.7376L38.8089 25.5483Z"
                fill="white"
              />
              <path
                d="M43.7448 28.9925L45.3335 28.054L44.0721 25.9186L44.29 24.503L48.1733 26.3764L50.0738 25.2536L44.801 22.7708L45.6019 17.7591L43.6977 18.884L42.9509 23.7945L42.8518 23.853L40.8946 20.5399L39.3059 21.4784L43.7448 28.9925Z"
                fill="white"
              />
              <path
                d="M51.0181 24.8315L56.4753 22.5951L55.8984 21.1874L52.1485 22.7241L51.3583 20.796L54.8125 19.3804L54.2356 17.9727L50.7815 19.3883L49.9929 17.464L53.727 15.9338L53.1501 14.5261L47.7086 16.756L51.0181 24.8315Z"
                fill="white"
              />
              <path
                d="M60.8509 21.315C63.4302 20.6683 64.5926 18.6811 63.9294 16.0357C63.2682 13.3986 61.3078 12.203 58.7534 12.8435L55.7277 13.6021L57.8501 22.0673L60.8509 21.315ZM59.2554 20.0851L57.9019 14.6869L59.0428 14.4008C60.63 14.0029 61.6722 14.6027 62.1437 16.4834C62.6173 18.3724 61.9834 19.4011 60.3921 19.8001L59.2554 20.0851Z"
                fill="white"
              />
              <path
                d="M68.7468 19.7555L70.5893 19.6559L70.4222 16.5666L71.7668 16.4939L73.5849 19.4939L75.6188 19.3839L73.5847 16.0969C74.5531 15.6178 75.0554 14.7243 74.9909 13.5329C74.8973 11.801 73.6924 10.7481 71.7137 10.855L68.2756 11.041L68.7468 19.7555ZM70.3421 15.0858L70.1995 12.4477L71.442 12.3805C72.5058 12.3229 73.0462 12.7674 73.0931 13.6355C73.1398 14.4993 72.6484 14.9611 71.5932 15.0182L70.3421 15.0858Z"
                fill="white"
              />
              <path
                d="M76.375 19.4258L82.2534 19.903L82.3764 18.3867L78.3372 18.0588L78.5058 15.9818L82.2265 16.2839L82.3496 14.7675L78.6289 14.4655L78.7971 12.3928L82.8194 12.7193L82.9425 11.203L77.0811 10.7272L76.375 19.4258Z"
                fill="white"
              />
              <path
                d="M92.484 16.2555C92.7078 14.2376 91.4873 12.7607 89.5208 12.2994C87.2764 11.7728 85.1912 12.9557 84.5391 15.7353C83.8889 18.5067 85.1995 20.4949 87.473 21.0283C89.6511 21.5393 91.3059 20.3999 91.8678 18.8816L90.0527 18.447C89.6694 19.3157 88.8332 19.671 87.8748 19.4462C86.5846 19.1435 85.934 17.9797 86.3603 16.1626C86.775 14.3952 87.8744 13.5896 89.1896 13.8981C90.1728 14.1288 90.7489 14.8636 90.6668 15.8292L92.484 16.2555Z"
                fill="white"
              />
              <path
                d="M97.555 14.842L95.8203 14.2133L92.8466 22.4183L94.5813 23.047L97.555 14.842Z"
                fill="white"
              />
              <path
                d="M96.1964 23.593L97.859 24.3932L99.0861 21.8436L100.484 22.5162C102.292 23.3866 103.864 22.814 104.62 21.2435C105.372 19.6807 104.869 18.0816 103.084 17.2223L99.9811 15.7291L96.1964 23.593ZM99.7274 20.5111L100.99 17.8886L102.111 18.4282C103.071 18.8902 103.284 19.636 102.907 20.4193C102.532 21.1988 101.809 21.5128 100.856 21.0545L99.7274 20.5111Z"
                fill="white"
              />
              <path
                d="M102.827 26.8674L107.756 30.1061L108.591 28.8347L105.204 26.6093L106.348 24.8678L109.468 26.9177L110.304 25.6463L107.184 23.5964L108.326 21.8584L111.698 24.0745L112.534 22.8031L107.619 19.5738L102.827 26.8674Z"
                fill="white"
              />
              <path
                d="M116.641 29.422L117.975 30.5833C118.977 29.3934 118.718 27.744 117.252 26.468C115.809 25.2117 114.058 25.0882 113.023 26.2894C112.175 27.2572 112.375 28.4092 113.255 29.6441L113.815 30.4481C114.381 31.2456 114.6 31.7983 114.217 32.2446C113.794 32.7239 113.053 32.6544 112.345 32.0388C111.622 31.4092 111.393 30.6217 111.921 29.9236L110.575 28.7512C109.37 30.2058 109.748 31.8288 111.346 33.2195C112.953 34.6185 114.566 34.6731 115.619 33.4706C116.57 32.3718 116.25 31.071 115.33 29.8177L114.862 29.1615C114.404 28.5316 114.11 27.9026 114.531 27.4385C114.901 27.0143 115.548 27.0299 116.229 27.6231C116.895 28.2023 117.04 28.8598 116.641 29.422Z"
                fill="white"
              />
              <rect
                x="38.8494"
                y="63.8494"
                width="50"
                height="50"
                transform="rotate(-30 38.8494 63.8494)"
                fill="url(#pattern0_0_1)"
              />
              <defs>
                <pattern
                  id="pattern0_0_1"
                  patternContentUnits="objectBoundingBox"
                  width="1"
                  height="1"
                >
                  <use xlinkHref="#image0_0_1" transform="scale(0.00625)" />
                </pattern>
                <image id="image0_0_1" width="160" height="160" />
              </defs>
            </svg>
          </div>

          <div className="hero_content">
            <button className="hotRecipes_btn">
              <img src="/hero/recipes.png" alt="recipes" />
              Hot Recipes
            </button>
            <h1>
              Spicy delicious <br />
              chicken wings
            </h1>
            <p className="paragraph">
              The marinade, featuring classic ingredients like garlic, ginger,
              and lemon juice, infuses the chicken wings with layers of spice
              and tanginess.
            </p>
            <div className="buttons_container">
              <button>
                <svg
                  width="24"
                  height="24"
                  viewBox="0 0 24 24"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    d="M9.75 1.5H14.25C14.4489 1.5 14.6397 1.42098 14.7803 1.28033C14.921 1.13968 15 0.948912 15 0.75C15 0.551088 14.921 0.360322 14.7803 0.21967C14.6397 0.0790176 14.4489 0 14.25 0L9.75 0C9.55109 0 9.36032 0.0790176 9.21967 0.21967C9.07902 0.360322 9 0.551088 9 0.75C9 0.948912 9.07902 1.13968 9.21967 1.28033C9.36032 1.42098 9.55109 1.5 9.75 1.5Z"
                    fill="black"
                  />
                  <path
                    d="M12 3C10.22 3 8.47991 3.52784 6.99987 4.51677C5.51983 5.50571 4.36628 6.91131 3.68509 8.55585C3.0039 10.2004 2.82567 12.01 3.17294 13.7558C3.5202 15.5016 4.37737 17.1053 5.63604 18.364C6.89472 19.6226 8.49836 20.4798 10.2442 20.8271C11.99 21.1743 13.7996 20.9961 15.4442 20.3149C17.0887 19.6337 18.4943 18.4802 19.4832 17.0001C20.4722 15.5201 21 13.78 21 12C20.9973 9.61388 20.0482 7.32626 18.361 5.63902C16.6737 3.95178 14.3861 3.0027 12 3ZM16.2426 8.81799L12.5304 12.5303C12.4607 12.6 12.378 12.6552 12.2871 12.6929C12.1961 12.7306 12.0985 12.75 12 12.75C11.9015 12.75 11.804 12.7306 11.713 12.6929C11.622 12.6553 11.5393 12.6 11.4697 12.5304C11.4 12.4607 11.3448 12.378 11.3071 12.2871C11.2694 12.1961 11.25 12.0985 11.25 12C11.25 11.9015 11.2694 11.804 11.3071 11.713C11.3448 11.622 11.4 11.5393 11.4696 11.4697L15.182 7.75736C15.2516 7.68771 15.3343 7.63246 15.4253 7.59477C15.5163 7.55708 15.6138 7.53767 15.7123 7.53767C15.8108 7.53767 15.9083 7.55707 15.9993 7.59476C16.0903 7.63244 16.173 7.68769 16.2426 7.75733C16.3123 7.82697 16.3675 7.90965 16.4052 8.00065C16.4429 8.09164 16.4623 8.18916 16.4623 8.28766C16.4623 8.38615 16.4429 8.48368 16.4052 8.57467C16.3675 8.66567 16.3123 8.74835 16.2427 8.81799H16.2426Z"
                    fill="black"
                  />
                </svg>
                30 Minutes
              </button>
              <button>
                <div>
                  <svg
                    width="9"
                    height="17"
                    viewBox="0 0 9 20"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      d="M8.99895 5.47887C8.99854 5.46532 8.99803 5.45177 8.99688 5.43813C8.99574 5.42444 8.99405 5.41094 8.99222 5.39748C8.99121 5.39052 8.99093 5.38361 8.98979 5.37661L8.23979 0.876609C8.22365 0.779407 8.18852 0.686338 8.1364 0.60272C8.08427 0.519102 8.01619 0.446573 7.93602 0.389279C7.85586 0.331985 7.7652 0.291047 7.66921 0.268807C7.57322 0.246566 7.47379 0.243457 7.3766 0.259659C7.2794 0.275861 7.18636 0.311055 7.10277 0.36323C7.01919 0.415405 6.9467 0.483539 6.88946 0.563738C6.83222 0.643938 6.79134 0.73463 6.76916 0.830634C6.74699 0.926638 6.74394 1.02607 6.76021 1.12325L7.36464 4.74993H5.625V0.99993C5.625 0.801018 5.54598 0.610252 5.40533 0.4696C5.26468 0.328948 5.07391 0.24993 4.875 0.24993C4.67609 0.24993 4.48532 0.328948 4.34467 0.4696C4.20402 0.610252 4.125 0.801018 4.125 0.99993V4.74993H2.38536L2.98979 1.12325C3.00606 1.02607 3.00301 0.926638 2.98083 0.830634C2.95866 0.73463 2.91778 0.643938 2.86053 0.563738C2.80329 0.483539 2.73081 0.415405 2.64722 0.36323C2.56364 0.311055 2.47059 0.275861 2.3734 0.259659C2.27621 0.243457 2.17678 0.246566 2.08079 0.268807C1.9848 0.291047 1.89413 0.331985 1.81397 0.389279C1.73381 0.446573 1.66572 0.519102 1.6136 0.60272C1.56148 0.686338 1.52635 0.779407 1.51021 0.876609L0.760208 5.37661C0.759064 5.38361 0.758789 5.39052 0.757782 5.39748C0.755951 5.41094 0.754257 5.42444 0.753112 5.43813C0.751969 5.45177 0.751464 5.46532 0.75105 5.47887C0.75087 5.48592 0.75 5.49284 0.75 5.49993C0.75 5.50661 0.750458 5.51316 0.750504 5.5198C0.750641 5.52442 0.750641 5.529 0.75087 5.53358C0.759927 6.49114 1.1014 7.4158 1.71687 8.14941C2.33235 8.88302 3.18359 9.38 4.125 9.55535V18.9999C4.125 19.1988 4.20402 19.3896 4.34467 19.5303C4.48532 19.6709 4.67609 19.7499 4.875 19.7499C5.07391 19.7499 5.26468 19.6709 5.40533 19.5303C5.54598 19.3896 5.625 19.1988 5.625 18.9999V9.55535C6.56641 9.38 7.41765 8.88302 8.03313 8.14941C8.6486 7.4158 8.99007 6.49113 8.99913 5.53358C8.99936 5.529 8.99936 5.52442 8.9995 5.51979C8.99954 5.51316 9 5.50661 9 5.49993C9 5.49284 8.99913 5.48592 8.99895 5.47887Z"
                      fill="black"
                    />
                  </svg>
                  <svg
                    width="7"
                    height="17"
                    viewBox="0 0 7 20"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      d="M6.87428 0.986725C6.87405 0.971256 6.87318 0.95578 6.87194 0.940261C6.87112 0.929733 6.87016 0.919296 6.86887 0.908905C6.86727 0.895951 6.86526 0.882995 6.86297 0.870041C6.86068 0.856995 6.85816 0.844085 6.85523 0.831314C6.85285 0.821152 6.8502 0.811036 6.8474 0.800918C6.84324 0.785858 6.83875 0.771027 6.83367 0.756378C6.83225 0.752167 6.83134 0.74791 6.82983 0.743698C6.82813 0.739166 6.82598 0.734955 6.82424 0.730469C6.81857 0.715774 6.81246 0.70134 6.80593 0.687165C6.80172 0.677915 6.79733 0.66879 6.79275 0.65979C6.78657 0.647705 6.78007 0.635819 6.77325 0.62413C6.76721 0.613648 6.76103 0.603348 6.75453 0.593231C6.74835 0.58371 6.74189 0.574463 6.7353 0.565216C6.72738 0.554046 6.71932 0.543106 6.71081 0.532486C6.70459 0.524658 6.69809 0.517105 6.69159 0.509552C6.6822 0.498749 6.67268 0.488083 6.66274 0.477874C6.65579 0.470733 6.64855 0.463867 6.64132 0.457001C6.63134 0.44748 6.62127 0.438141 6.61079 0.429214C6.60237 0.422027 6.59362 0.415253 6.58488 0.408432C6.57508 0.400833 6.56524 0.39328 6.55508 0.386139C6.54469 0.37886 6.53398 0.37204 6.52322 0.365311C6.51411 0.359592 6.505 0.353821 6.49562 0.348511C6.48339 0.341552 6.4709 0.335144 6.45826 0.328873C6.44961 0.32457 6.441 0.320267 6.43217 0.316284C6.41889 0.310333 6.40539 0.304932 6.3917 0.299713C6.38264 0.29628 6.37362 0.292847 6.36437 0.289734C6.35128 0.285293 6.33796 0.281403 6.32455 0.277695C6.31384 0.274719 6.30317 0.271927 6.29237 0.269409C6.28047 0.266708 6.26843 0.264374 6.25634 0.262222C6.24316 0.259842 6.23002 0.257828 6.2167 0.25618C6.20649 0.254943 6.19624 0.253936 6.18589 0.253066C6.17056 0.251831 6.15513 0.251053 6.13961 0.250732C6.13467 0.250641 6.12991 0.25 6.12496 0.25C6.12043 0.25 6.11599 0.250595 6.1115 0.250686C6.09612 0.250961 6.08074 0.251785 6.06532 0.253022C6.05474 0.253846 6.04426 0.254803 6.03382 0.256088C6.02087 0.25769 6.00796 0.259705 5.995 0.261994C5.98196 0.264283 5.96905 0.266801 5.95628 0.269776C5.94612 0.27211 5.936 0.274765 5.92588 0.277557C5.91082 0.281676 5.89599 0.286208 5.88134 0.291289C5.87713 0.292709 5.87287 0.293624 5.86866 0.295133C5.85941 0.29852 5.8503 0.302549 5.8411 0.305982C5.83762 0.3074 5.83405 0.308637 5.83057 0.310101C3.8984 1.03625 2.35107 3.4066 1.23147 7.35806C0.723603 9.18144 0.355139 11.0408 0.129266 12.9201C0.118059 13.0247 0.128994 13.1305 0.161359 13.2306C0.193725 13.3308 0.246798 13.423 0.317128 13.5012C0.387458 13.5795 0.473472 13.6421 0.569579 13.685C0.665685 13.7278 0.769734 13.75 0.874963 13.75H5.37496V19C5.37496 19.1989 5.45398 19.3897 5.59463 19.5303C5.73528 19.671 5.92605 19.75 6.12496 19.75C6.32387 19.75 6.51464 19.671 6.65529 19.5303C6.79595 19.3897 6.87496 19.1989 6.87496 19V1C6.87496 0.995514 6.87437 0.991211 6.87428 0.986725Z"
                      fill="black"
                    />
                  </svg>
                </div>
                Chicken
              </button>
            </div>

            <div className="recipes_info">
              <div className="chef_info">
                <div>
                  <img src="/hero/Ellipse 2.png" alt="Ellipse" />
                </div>
                <div>
                  <div className="name">John Smith</div>
                  <div className="date">15 March 2022</div>
                </div>
              </div>
              <button>
                View Recipes
                <svg
                  width="20"
                  height="20"
                  viewBox="0 0 20 20"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    d="M10 0.25C8.07164 0.25 6.18657 0.821828 4.58319 1.89317C2.97982 2.96451 1.73013 4.48726 0.992179 6.26884C0.254225 8.05042 0.061142 10.0108 0.437348 11.9021C0.813554 13.7934 1.74215 15.5307 3.10571 16.8943C4.46928 18.2579 6.20656 19.1865 8.09787 19.5627C9.98919 19.9389 11.9496 19.7458 13.7312 19.0078C15.5127 18.2699 17.0355 17.0202 18.1068 15.4168C19.1782 13.8134 19.75 11.9284 19.75 10C19.747 7.41506 18.7188 4.93684 16.891 3.10901C15.0632 1.28118 12.5849 0.252988 10 0.25ZM13.416 10.624L8.91602 13.624C8.8031 13.6995 8.67181 13.7428 8.53617 13.7494C8.40054 13.756 8.26565 13.7257 8.14592 13.6616C8.02619 13.5975 7.92612 13.5021 7.85639 13.3856C7.78665 13.2691 7.74988 13.1358 7.75 13V7C7.74988 6.8642 7.78665 6.73092 7.85639 6.6144C7.92612 6.49787 8.02619 6.40248 8.14592 6.3384C8.26565 6.27433 8.40054 6.24398 8.53617 6.2506C8.67181 6.25721 8.8031 6.30055 8.91602 6.37598L13.416 9.37598C13.5188 9.44446 13.603 9.53725 13.6613 9.64611C13.7195 9.75497 13.75 9.87653 13.75 10C13.75 10.1235 13.7195 10.245 13.6613 10.3539C13.603 10.4627 13.5188 10.5555 13.416 10.624Z"
                    fill="white"
                  />
                </svg>
              </button>
            </div>
          </div>
          <div className="hero_image">
            <img src="/hero/hero.jfif" alt="Hero" />
          </div>
        </div>
      </div>

      {/* Categories */}
      <div>
        <div className="categories">
          <div className="categories_header">
            <h2>Categories</h2>
            <button>View All Categories</button>
          </div>
          <div className="all_categories">
            {categories.map((category, index) => (
              <div className={`category${index + 1}`} key={index}>
                <img src={category.src} alt={category.title} className="img2" />
                <img src={category.src} alt={category.title} />
                <p>{category.title}</p>
              </div>
            ))}
          </div>
        </div>
      </div>

      {/* Simple recipes */}
      <div className="simple_recipes">
        <h2>Simple and tasty recipes</h2>
        <p className="paragraph">
          Lorem ipsum dolor sit amet consectetur adipisicing elit. Fugit nulla,
          maxime culpa dicta odio eius provident molestiae perferendis qui nobis
          cumque dignissimos.
        </p>
        <div className="box">
          {simpleRecipes.map((recipe, index) => (
            <div className="card" key={index}>
              <img src={recipe.src} alt={recipe.title} />
              <h3>{recipe.title}</h3>
              <div className="buttons_container">
                <button>
                  <svg
                    width="24"
                    height="24"
                    viewBox="0 0 24 24"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      d="M9.75 1.5H14.25C14.4489 1.5 14.6397 1.42098 14.7803 1.28033C14.921 1.13968 15 0.948912 15 0.75C15 0.551088 14.921 0.360322 14.7803 0.21967C14.6397 0.0790176 14.4489 0 14.25 0L9.75 0C9.55109 0 9.36032 0.0790176 9.21967 0.21967C9.07902 0.360322 9 0.551088 9 0.75C9 0.948912 9.07902 1.13968 9.21967 1.28033C9.36032 1.42098 9.55109 1.5 9.75 1.5Z"
                      fill="black"
                    />
                    <path
                      d="M12 3C10.22 3 8.47991 3.52784 6.99987 4.51677C5.51983 5.50571 4.36628 6.91131 3.68509 8.55585C3.0039 10.2004 2.82567 12.01 3.17294 13.7558C3.5202 15.5016 4.37737 17.1053 5.63604 18.364C6.89472 19.6226 8.49836 20.4798 10.2442 20.8271C11.99 21.1743 13.7996 20.9961 15.4442 20.3149C17.0887 19.6337 18.4943 18.4802 19.4832 17.0001C20.4722 15.5201 21 13.78 21 12C20.9973 9.61388 20.0482 7.32626 18.361 5.63902C16.6737 3.95178 14.3861 3.0027 12 3ZM16.2426 8.81799L12.5304 12.5303C12.4607 12.6 12.378 12.6552 12.2871 12.6929C12.1961 12.7306 12.0985 12.75 12 12.75C11.9015 12.75 11.804 12.7306 11.713 12.6929C11.622 12.6553 11.5393 12.6 11.4697 12.5304C11.4 12.4607 11.3448 12.378 11.3071 12.2871C11.2694 12.1961 11.25 12.0985 11.25 12C11.25 11.9015 11.2694 11.804 11.3071 11.713C11.3448 11.622 11.4 11.5393 11.4696 11.4697L15.182 7.75736C15.2516 7.68771 15.3343 7.63246 15.4253 7.59477C15.5163 7.55708 15.6138 7.53767 15.7123 7.53767C15.8108 7.53767 15.9083 7.55707 15.9993 7.59476C16.0903 7.63244 16.173 7.68769 16.2426 7.75733C16.3123 7.82697 16.3675 7.90965 16.4052 8.00065C16.4429 8.09164 16.4623 8.18916 16.4623 8.28766C16.4623 8.38615 16.4429 8.48368 16.4052 8.57467C16.3675 8.66567 16.3123 8.74835 16.2427 8.81799H16.2426Z"
                      fill="black"
                    />
                  </svg>
                  30 Minutes
                </button>
                <button>
                  <div>
                    <svg
                      width="9"
                      height="17"
                      viewBox="0 0 9 20"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path
                        d="M8.99895 5.47887C8.99854 5.46532 8.99803 5.45177 8.99688 5.43813C8.99574 5.42444 8.99405 5.41094 8.99222 5.39748C8.99121 5.39052 8.99093 5.38361 8.98979 5.37661L8.23979 0.876609C8.22365 0.779407 8.18852 0.686338 8.1364 0.60272C8.08427 0.519102 8.01619 0.446573 7.93602 0.389279C7.85586 0.331985 7.7652 0.291047 7.66921 0.268807C7.57322 0.246566 7.47379 0.243457 7.3766 0.259659C7.2794 0.275861 7.18636 0.311055 7.10277 0.36323C7.01919 0.415405 6.9467 0.483539 6.88946 0.563738C6.83222 0.643938 6.79134 0.73463 6.76916 0.830634C6.74699 0.926638 6.74394 1.02607 6.76021 1.12325L7.36464 4.74993H5.625V0.99993C5.625 0.801018 5.54598 0.610252 5.40533 0.4696C5.26468 0.328948 5.07391 0.24993 4.875 0.24993C4.67609 0.24993 4.48532 0.328948 4.34467 0.4696C4.20402 0.610252 4.125 0.801018 4.125 0.99993V4.74993H2.38536L2.98979 1.12325C3.00606 1.02607 3.00301 0.926638 2.98083 0.830634C2.95866 0.73463 2.91778 0.643938 2.86053 0.563738C2.80329 0.483539 2.73081 0.415405 2.64722 0.36323C2.56364 0.311055 2.47059 0.275861 2.3734 0.259659C2.27621 0.243457 2.17678 0.246566 2.08079 0.268807C1.9848 0.291047 1.89413 0.331985 1.81397 0.389279C1.73381 0.446573 1.66572 0.519102 1.6136 0.60272C1.56148 0.686338 1.52635 0.779407 1.51021 0.876609L0.760208 5.37661C0.759064 5.38361 0.758789 5.39052 0.757782 5.39748C0.755951 5.41094 0.754257 5.42444 0.753112 5.43813C0.751969 5.45177 0.751464 5.46532 0.75105 5.47887C0.75087 5.48592 0.75 5.49284 0.75 5.49993C0.75 5.50661 0.750458 5.51316 0.750504 5.5198C0.750641 5.52442 0.750641 5.529 0.75087 5.53358C0.759927 6.49114 1.1014 7.4158 1.71687 8.14941C2.33235 8.88302 3.18359 9.38 4.125 9.55535V18.9999C4.125 19.1988 4.20402 19.3896 4.34467 19.5303C4.48532 19.6709 4.67609 19.7499 4.875 19.7499C5.07391 19.7499 5.26468 19.6709 5.40533 19.5303C5.54598 19.3896 5.625 19.1988 5.625 18.9999V9.55535C6.56641 9.38 7.41765 8.88302 8.03313 8.14941C8.6486 7.4158 8.99007 6.49113 8.99913 5.53358C8.99936 5.529 8.99936 5.52442 8.9995 5.51979C8.99954 5.51316 9 5.50661 9 5.49993C9 5.49284 8.99913 5.48592 8.99895 5.47887Z"
                        fill="black"
                      />
                    </svg>
                    <svg
                      width="7"
                      height="17"
                      viewBox="0 0 7 20"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path
                        d="M6.87428 0.986725C6.87405 0.971256 6.87318 0.95578 6.87194 0.940261C6.87112 0.929733 6.87016 0.919296 6.86887 0.908905C6.86727 0.895951 6.86526 0.882995 6.86297 0.870041C6.86068 0.856995 6.85816 0.844085 6.85523 0.831314C6.85285 0.821152 6.8502 0.811036 6.8474 0.800918C6.84324 0.785858 6.83875 0.771027 6.83367 0.756378C6.83225 0.752167 6.83134 0.74791 6.82983 0.743698C6.82813 0.739166 6.82598 0.734955 6.82424 0.730469C6.81857 0.715774 6.81246 0.70134 6.80593 0.687165C6.80172 0.677915 6.79733 0.66879 6.79275 0.65979C6.78657 0.647705 6.78007 0.635819 6.77325 0.62413C6.76721 0.613648 6.76103 0.603348 6.75453 0.593231C6.74835 0.58371 6.74189 0.574463 6.7353 0.565216C6.72738 0.554046 6.71932 0.543106 6.71081 0.532486C6.70459 0.524658 6.69809 0.517105 6.69159 0.509552C6.6822 0.498749 6.67268 0.488083 6.66274 0.477874C6.65579 0.470733 6.64855 0.463867 6.64132 0.457001C6.63134 0.44748 6.62127 0.438141 6.61079 0.429214C6.60237 0.422027 6.59362 0.415253 6.58488 0.408432C6.57508 0.400833 6.56524 0.39328 6.55508 0.386139C6.54469 0.37886 6.53398 0.37204 6.52322 0.365311C6.51411 0.359592 6.505 0.353821 6.49562 0.348511C6.48339 0.341552 6.4709 0.335144 6.45826 0.328873C6.44961 0.32457 6.441 0.320267 6.43217 0.316284C6.41889 0.310333 6.40539 0.304932 6.3917 0.299713C6.38264 0.29628 6.37362 0.292847 6.36437 0.289734C6.35128 0.285293 6.33796 0.281403 6.32455 0.277695C6.31384 0.274719 6.30317 0.271927 6.29237 0.269409C6.28047 0.266708 6.26843 0.264374 6.25634 0.262222C6.24316 0.259842 6.23002 0.257828 6.2167 0.25618C6.20649 0.254943 6.19624 0.253936 6.18589 0.253066C6.17056 0.251831 6.15513 0.251053 6.13961 0.250732C6.13467 0.250641 6.12991 0.25 6.12496 0.25C6.12043 0.25 6.11599 0.250595 6.1115 0.250686C6.09612 0.250961 6.08074 0.251785 6.06532 0.253022C6.05474 0.253846 6.04426 0.254803 6.03382 0.256088C6.02087 0.25769 6.00796 0.259705 5.995 0.261994C5.98196 0.264283 5.96905 0.266801 5.95628 0.269776C5.94612 0.27211 5.936 0.274765 5.92588 0.277557C5.91082 0.281676 5.89599 0.286208 5.88134 0.291289C5.87713 0.292709 5.87287 0.293624 5.86866 0.295133C5.85941 0.29852 5.8503 0.302549 5.8411 0.305982C5.83762 0.3074 5.83405 0.308637 5.83057 0.310101C3.8984 1.03625 2.35107 3.4066 1.23147 7.35806C0.723603 9.18144 0.355139 11.0408 0.129266 12.9201C0.118059 13.0247 0.128994 13.1305 0.161359 13.2306C0.193725 13.3308 0.246798 13.423 0.317128 13.5012C0.387458 13.5795 0.473472 13.6421 0.569579 13.685C0.665685 13.7278 0.769734 13.75 0.874963 13.75H5.37496V19C5.37496 19.1989 5.45398 19.3897 5.59463 19.5303C5.73528 19.671 5.92605 19.75 6.12496 19.75C6.32387 19.75 6.51464 19.671 6.65529 19.5303C6.79595 19.3897 6.87496 19.1989 6.87496 19V1C6.87496 0.995514 6.87437 0.991211 6.87428 0.986725Z"
                        fill="black"
                      />
                    </svg>
                  </div>
                  {recipe.category}
                </button>
              </div>
              <div className="favorite">
                <GoHeartFill size={20} fill="rgba(255, 99, 99, 1)" />
                {/* <GoHeartFill size={20} fill="#cad0d3" /> */}
              </div>
            </div>
          ))}
        </div>
      </div>
      {/* BeChef */}
      <div className="beChef_container">
        <div className="beChef">
          <div className="beChef_content">
            <h1>
              Everyone can be a
              <br />
              chef in their own kitchen
            </h1>
            <p className="paragraph">
              Lorem ipsum dolor sit amet, consectetuipisicing elit, sed do
              eiusmod tempor incididunt ut labore et dolore magna aliqut enim ad
              minim
            </p>
            <button className="learnMore_btn">Learn More</button>
          </div>
          <div className="beChef_image">
            <img
              src="/portrait-happy-male-chef-dressed-uniform 1.png"
              alt="Be Chef"
            />
          </div>
        </div>
      </div>

      {/* Instagram*/}
      <div className="instagram">
        <div className="instagram_header">
          <h2>Check out @foodieland on Instagram</h2>
          <p className="paragraph">
            Lorem ipsum dolor sit amet consectetur adipisicing elit. Fugit
            nulla, maxime culpa dicta odio eius provident molestiae perferendis
            qui nobis cumque dignissimos.
          </p>
        </div>
        <div className="box">
          {posts.map((recipe, index) => (
            <div className="card" key={index}>
              <div className="card_header">
                <div className="card_header_left">
                  <img src="/instagram/Oval.png" alt="" />
                  <div className="card_header_info">
                    <div>
                      <h5>Foodieland.</h5>
                      <img src="/instagram/Official Icon.png" alt="" />
                    </div>
                    <p>Tokyo, Japan</p>
                  </div>
                </div>
                <div>...</div>
              </div>

              <img src={recipe.src} alt={recipe.title} />
              <div className="content">
                <ul>
                  <li className="actions">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="20"
                      height="17"
                      viewBox="0 0 20 17"
                      fill="none"
                    >
                      <path
                        fill-rule="evenodd"
                        clip-rule="evenodd"
                        d="M10.0081 2.25665L9.5015 1.66459C7.46296 -0.373949 4.21201 -0.235034 2.17347 1.8035C0.134936 3.84204 0.527258 7.7832 2.56579 9.82174C3.39315 10.6491 4.63217 11.7946 6.28285 13.2584L7.67155 14.4801L9.23129 15.834C9.66075 16.2046 10.2953 16.2103 10.7313 15.8474L11.9948 14.788C14.2076 12.918 15.8496 11.445 16.9209 10.3692L17.1989 10.0856L17.4486 9.82174C19.3471 7.77441 19.835 3.79463 17.8439 1.8035L17.6996 1.66459C15.6523 -0.233931 12.5651 -0.326542 10.5739 1.66459L10.0081 2.25665ZM2.99374 2.6236C4.6057 1.01164 7.09948 0.936068 8.65131 2.45503L9.96057 3.98519L11.4126 2.4659C12.9019 0.97698 15.2755 0.998504 16.9109 2.51501L17.0392 2.63889C18.4463 4.0462 18.2069 7.29787 16.598 9.03284L16.3564 9.28814L16.0882 9.56149C15.2002 10.4528 13.884 11.6468 12.1487 13.1339L11.2461 13.9019L9.98924 14.9557L8.43197 13.6039L7.31742 12.6249C5.74693 11.2368 4.5354 10.1266 3.68713 9.29861L3.38606 9.00135C1.70674 7.32202 1.4967 4.12064 2.99374 2.6236Z"
                        fill="#262626"
                      />
                    </svg>
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="18"
                      height="18"
                      viewBox="0 0 18 18"
                      fill="none"
                    >
                      <path
                        fill-rule="evenodd"
                        clip-rule="evenodd"
                        d="M17.5467 8.70686C17.5467 4.00876 13.7381 0.200195 9.03999 0.200195C4.34189 0.200195 0.533325 4.00876 0.533325 8.70686C0.533325 13.405 4.34189 17.2135 9.03999 17.2135C10.0654 17.2135 11.474 16.8395 13.2659 16.0913L16.1634 17.2216L16.2493 17.2495C16.6238 17.3477 17.0207 17.1516 17.1649 16.7822C17.2225 16.6344 17.2333 16.4725 17.1958 16.3184L16.3879 12.9958L16.5202 12.6792C17.2045 11.0124 17.5467 9.68828 17.5467 8.70686ZM15.4365 12.2643L15.1699 12.8957L15.8933 15.8709L13.2511 14.8402L12.5295 15.1395C11.0078 15.7536 9.8323 16.0533 9.04 16.0533C4.98255 16.0533 1.69333 12.7641 1.69333 8.70667C1.69333 4.64921 4.98255 1.36 9.04 1.36C13.0975 1.36 16.3867 4.64921 16.3867 8.70667C16.3867 9.5152 16.075 10.7137 15.4365 12.2643Z"
                        fill="#262626"
                      />
                    </svg>
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="18"
                      height="16"
                      viewBox="0 0 18 16"
                      fill="none"
                    >
                      <path
                        fill-rule="evenodd"
                        clip-rule="evenodd"
                        d="M17.5748 1.0732L9.31158 15.1893C9.05142 15.6337 8.38425 15.5439 8.25083 15.0465L6.12389 7.11552L6.10635 7.09073C6.09055 7.06337 6.07724 7.03525 6.06634 7.00665L0.250997 1.19032C-0.114383 0.824938 0.144394 0.200195 0.661119 0.200195H17.0743C17.5223 0.200195 17.8011 0.686548 17.5748 1.0732ZM15.4866 2.34366L7.31125 7.0637L9.01164 13.4051L15.4866 2.34366ZM2.06135 1.35997L14.8718 1.35924L6.74999 6.04835L2.06135 1.35997Z"
                        fill="#262626"
                      />
                    </svg>
                  </li>
                  <li className="dots">
                    <span className="blueDot">
                      <GoDotFill />
                    </span>
                    <span className="grayDot">
                      <GoDotFill />
                    </span>
                    <span className="grayDot">
                      <GoDotFill />
                    </span>
                  </li>
                  <li>
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="16"
                      height="19"
                      viewBox="0 0 16 19"
                      fill="none"
                    >
                      <path
                        fill-rule="evenodd"
                        clip-rule="evenodd"
                        d="M0.0522461 0.951852V17.6964C0.0522461 17.8996 0.132235 18.0946 0.274915 18.2393C0.574787 18.5435 1.06442 18.5469 1.36855 18.247L7.97949 11.7285L14.5904 18.247C14.7351 18.3897 14.9302 18.4697 15.1334 18.4697C15.5605 18.4697 15.9067 18.1235 15.9067 17.6964V0.951852C15.9067 0.738302 15.7336 0.565186 15.5201 0.565186H0.438913C0.225363 0.565186 0.0522461 0.738302 0.0522461 0.951852ZM1.21223 16.772V1.72511H14.7467V16.772L8.52244 10.6348C8.22134 10.3379 7.7376 10.3379 7.4365 10.6348L1.21223 16.772Z"
                        fill="#262626"
                      />
                    </svg>
                  </li>
                </ul>
                <div className="likes">
                  <img src="/instagram/profile.png" alt="" />
                  Liked by <strong>craig_love</strong> and{" "}
                  <strong>44,686</strong> others
                </div>
                <div>
                  <strong>Foodieland.</strong> The vegetables dishes need to
                  have certain vitamin for those people
                </div>
                <small>September 19</small>
              </div>
              <div className="slides">1/3</div>
            </div>
          ))}
        </div>
      </div>

      {/* Delicious recipes */}
      <div className="delicious_recipes">
        <div className="delicious_recipes_header">
          <h2>Try this delicious recipe to make your day</h2>
          <p className="paragraph">
            Lorem ipsum dolor sit amet consectetur adipisicing elit. Fugit
            nulla, maxime culpa dicta odio eius provident molestiae perferendis
            qui nobis cumque dignissimos.
          </p>
        </div>
        <div className="box">
          {deliciousRecipes.map((recipe, index) => (
            <div className="card" key={index}>
              <img src={recipe.src} alt={recipe.title} />
              <h4>Big and Juicy Wagyu Beef Cheeseburger</h4>
              <div className="buttons_container">
                <button>
                  <svg
                    width="24"
                    height="24"
                    viewBox="0 0 24 24"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      d="M9.75 1.5H14.25C14.4489 1.5 14.6397 1.42098 14.7803 1.28033C14.921 1.13968 15 0.948912 15 0.75C15 0.551088 14.921 0.360322 14.7803 0.21967C14.6397 0.0790176 14.4489 0 14.25 0L9.75 0C9.55109 0 9.36032 0.0790176 9.21967 0.21967C9.07902 0.360322 9 0.551088 9 0.75C9 0.948912 9.07902 1.13968 9.21967 1.28033C9.36032 1.42098 9.55109 1.5 9.75 1.5Z"
                      fill="black"
                    />
                    <path
                      d="M12 3C10.22 3 8.47991 3.52784 6.99987 4.51677C5.51983 5.50571 4.36628 6.91131 3.68509 8.55585C3.0039 10.2004 2.82567 12.01 3.17294 13.7558C3.5202 15.5016 4.37737 17.1053 5.63604 18.364C6.89472 19.6226 8.49836 20.4798 10.2442 20.8271C11.99 21.1743 13.7996 20.9961 15.4442 20.3149C17.0887 19.6337 18.4943 18.4802 19.4832 17.0001C20.4722 15.5201 21 13.78 21 12C20.9973 9.61388 20.0482 7.32626 18.361 5.63902C16.6737 3.95178 14.3861 3.0027 12 3ZM16.2426 8.81799L12.5304 12.5303C12.4607 12.6 12.378 12.6552 12.2871 12.6929C12.1961 12.7306 12.0985 12.75 12 12.75C11.9015 12.75 11.804 12.7306 11.713 12.6929C11.622 12.6553 11.5393 12.6 11.4697 12.5304C11.4 12.4607 11.3448 12.378 11.3071 12.2871C11.2694 12.1961 11.25 12.0985 11.25 12C11.25 11.9015 11.2694 11.804 11.3071 11.713C11.3448 11.622 11.4 11.5393 11.4696 11.4697L15.182 7.75736C15.2516 7.68771 15.3343 7.63246 15.4253 7.59477C15.5163 7.55708 15.6138 7.53767 15.7123 7.53767C15.8108 7.53767 15.9083 7.55707 15.9993 7.59476C16.0903 7.63244 16.173 7.68769 16.2426 7.75733C16.3123 7.82697 16.3675 7.90965 16.4052 8.00065C16.4429 8.09164 16.4623 8.18916 16.4623 8.28766C16.4623 8.38615 16.4429 8.48368 16.4052 8.57467C16.3675 8.66567 16.3123 8.74835 16.2427 8.81799H16.2426Z"
                      fill="black"
                    />
                  </svg>
                  30 Minutes
                </button>
                <button>
                  <div>
                    <svg
                      width="9"
                      height="17"
                      viewBox="0 0 9 20"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path
                        d="M8.99895 5.47887C8.99854 5.46532 8.99803 5.45177 8.99688 5.43813C8.99574 5.42444 8.99405 5.41094 8.99222 5.39748C8.99121 5.39052 8.99093 5.38361 8.98979 5.37661L8.23979 0.876609C8.22365 0.779407 8.18852 0.686338 8.1364 0.60272C8.08427 0.519102 8.01619 0.446573 7.93602 0.389279C7.85586 0.331985 7.7652 0.291047 7.66921 0.268807C7.57322 0.246566 7.47379 0.243457 7.3766 0.259659C7.2794 0.275861 7.18636 0.311055 7.10277 0.36323C7.01919 0.415405 6.9467 0.483539 6.88946 0.563738C6.83222 0.643938 6.79134 0.73463 6.76916 0.830634C6.74699 0.926638 6.74394 1.02607 6.76021 1.12325L7.36464 4.74993H5.625V0.99993C5.625 0.801018 5.54598 0.610252 5.40533 0.4696C5.26468 0.328948 5.07391 0.24993 4.875 0.24993C4.67609 0.24993 4.48532 0.328948 4.34467 0.4696C4.20402 0.610252 4.125 0.801018 4.125 0.99993V4.74993H2.38536L2.98979 1.12325C3.00606 1.02607 3.00301 0.926638 2.98083 0.830634C2.95866 0.73463 2.91778 0.643938 2.86053 0.563738C2.80329 0.483539 2.73081 0.415405 2.64722 0.36323C2.56364 0.311055 2.47059 0.275861 2.3734 0.259659C2.27621 0.243457 2.17678 0.246566 2.08079 0.268807C1.9848 0.291047 1.89413 0.331985 1.81397 0.389279C1.73381 0.446573 1.66572 0.519102 1.6136 0.60272C1.56148 0.686338 1.52635 0.779407 1.51021 0.876609L0.760208 5.37661C0.759064 5.38361 0.758789 5.39052 0.757782 5.39748C0.755951 5.41094 0.754257 5.42444 0.753112 5.43813C0.751969 5.45177 0.751464 5.46532 0.75105 5.47887C0.75087 5.48592 0.75 5.49284 0.75 5.49993C0.75 5.50661 0.750458 5.51316 0.750504 5.5198C0.750641 5.52442 0.750641 5.529 0.75087 5.53358C0.759927 6.49114 1.1014 7.4158 1.71687 8.14941C2.33235 8.88302 3.18359 9.38 4.125 9.55535V18.9999C4.125 19.1988 4.20402 19.3896 4.34467 19.5303C4.48532 19.6709 4.67609 19.7499 4.875 19.7499C5.07391 19.7499 5.26468 19.6709 5.40533 19.5303C5.54598 19.3896 5.625 19.1988 5.625 18.9999V9.55535C6.56641 9.38 7.41765 8.88302 8.03313 8.14941C8.6486 7.4158 8.99007 6.49113 8.99913 5.53358C8.99936 5.529 8.99936 5.52442 8.9995 5.51979C8.99954 5.51316 9 5.50661 9 5.49993C9 5.49284 8.99913 5.48592 8.99895 5.47887Z"
                        fill="black"
                      />
                    </svg>
                    <svg
                      width="7"
                      height="17"
                      viewBox="0 0 7 20"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path
                        d="M6.87428 0.986725C6.87405 0.971256 6.87318 0.95578 6.87194 0.940261C6.87112 0.929733 6.87016 0.919296 6.86887 0.908905C6.86727 0.895951 6.86526 0.882995 6.86297 0.870041C6.86068 0.856995 6.85816 0.844085 6.85523 0.831314C6.85285 0.821152 6.8502 0.811036 6.8474 0.800918C6.84324 0.785858 6.83875 0.771027 6.83367 0.756378C6.83225 0.752167 6.83134 0.74791 6.82983 0.743698C6.82813 0.739166 6.82598 0.734955 6.82424 0.730469C6.81857 0.715774 6.81246 0.70134 6.80593 0.687165C6.80172 0.677915 6.79733 0.66879 6.79275 0.65979C6.78657 0.647705 6.78007 0.635819 6.77325 0.62413C6.76721 0.613648 6.76103 0.603348 6.75453 0.593231C6.74835 0.58371 6.74189 0.574463 6.7353 0.565216C6.72738 0.554046 6.71932 0.543106 6.71081 0.532486C6.70459 0.524658 6.69809 0.517105 6.69159 0.509552C6.6822 0.498749 6.67268 0.488083 6.66274 0.477874C6.65579 0.470733 6.64855 0.463867 6.64132 0.457001C6.63134 0.44748 6.62127 0.438141 6.61079 0.429214C6.60237 0.422027 6.59362 0.415253 6.58488 0.408432C6.57508 0.400833 6.56524 0.39328 6.55508 0.386139C6.54469 0.37886 6.53398 0.37204 6.52322 0.365311C6.51411 0.359592 6.505 0.353821 6.49562 0.348511C6.48339 0.341552 6.4709 0.335144 6.45826 0.328873C6.44961 0.32457 6.441 0.320267 6.43217 0.316284C6.41889 0.310333 6.40539 0.304932 6.3917 0.299713C6.38264 0.29628 6.37362 0.292847 6.36437 0.289734C6.35128 0.285293 6.33796 0.281403 6.32455 0.277695C6.31384 0.274719 6.30317 0.271927 6.29237 0.269409C6.28047 0.266708 6.26843 0.264374 6.25634 0.262222C6.24316 0.259842 6.23002 0.257828 6.2167 0.25618C6.20649 0.254943 6.19624 0.253936 6.18589 0.253066C6.17056 0.251831 6.15513 0.251053 6.13961 0.250732C6.13467 0.250641 6.12991 0.25 6.12496 0.25C6.12043 0.25 6.11599 0.250595 6.1115 0.250686C6.09612 0.250961 6.08074 0.251785 6.06532 0.253022C6.05474 0.253846 6.04426 0.254803 6.03382 0.256088C6.02087 0.25769 6.00796 0.259705 5.995 0.261994C5.98196 0.264283 5.96905 0.266801 5.95628 0.269776C5.94612 0.27211 5.936 0.274765 5.92588 0.277557C5.91082 0.281676 5.89599 0.286208 5.88134 0.291289C5.87713 0.292709 5.87287 0.293624 5.86866 0.295133C5.85941 0.29852 5.8503 0.302549 5.8411 0.305982C5.83762 0.3074 5.83405 0.308637 5.83057 0.310101C3.8984 1.03625 2.35107 3.4066 1.23147 7.35806C0.723603 9.18144 0.355139 11.0408 0.129266 12.9201C0.118059 13.0247 0.128994 13.1305 0.161359 13.2306C0.193725 13.3308 0.246798 13.423 0.317128 13.5012C0.387458 13.5795 0.473472 13.6421 0.569579 13.685C0.665685 13.7278 0.769734 13.75 0.874963 13.75H5.37496V19C5.37496 19.1989 5.45398 19.3897 5.59463 19.5303C5.73528 19.671 5.92605 19.75 6.12496 19.75C6.32387 19.75 6.51464 19.671 6.65529 19.5303C6.79595 19.3897 6.87496 19.1989 6.87496 19V1C6.87496 0.995514 6.87437 0.991211 6.87428 0.986725Z"
                        fill="black"
                      />
                    </svg>
                  </div>
                  {recipe.category}
                </button>
              </div>
              <div className="favorite">
                {/* <GoHeartFill size={18} fill="rgba(255, 99, 99, 1)" /> */}
                <GoHeartFill size={18} fill="#c4cbce" />
              </div>
            </div>
          ))}
        </div>
      </div>

      <div className="inbox">
        <div className="inbox_header">
          <h2>Deliciousness to your inbox</h2>
          <p className="paragraph">
            Lorem ipsum dolor sit amet, consectetuipisicing elit, sed do eiusmod
            tempor incididunt ut labore et dolore magna aliqut enim ad minim
          </p>
        </div>
        <form>
          <input type="text" placeholder="Your email address..." />
          <button>Subscribe</button>
        </form>
        <div className="plate">
          <img src="/inbox/plate.png" alt="Plate" />
        </div>
        <div className="salad">
          <img src="/inbox/salad.png" alt="Salad" />
        </div>
        <div className="rucola">
          <img src="/inbox/rucola.png" alt="Rucola" />
        </div>
      </div>

      <footer>
        <div>
          <div className="logo">
            <h6>
              Foodieland<span>.</span>
            </h6>
            <p>Lorem ipsum dolor sit amet, consectetuipisicing elit, </p>
          </div>
          <ul>
            <li>
              <a href="">Recipes</a>
            </li>
            <li>
              <a href="">Blog</a>
            </li>
            <li>
              <a href="">Contact</a>
            </li>
            <li>
              <a href="">About us</a>
            </li>
          </ul>
        </div>
        <div className="copyright">
          <div></div>
          <p>
            © 2020 Flowbase. Powered by <span>Webflow</span>
          </p>
          <Socials />
        </div>
      </footer>
    </div>
  );
}
